...
//    private Data data  = null;	
	...
	public RowFilterTree(CommonTree tree, ReadData  rp)
	{
		try {
			this.rp= rp;
	 ...
	 public boolean process( ) throws Exception
	 {
	    ...
	    private   boolean process(Node node) throws Exception
		 ...
		 private boolean  atomByteProcess(Node node) throws Exception
		 {
			  ...
			  byte tempValue = rp.getColumnData(node.columnName,rowData)[0];
			  int value = 0;
			  if(tempValue < 0)
			  {
...
//					  		System.out.println("Indexfilter is " + value +"node.inValues.get(i) is  " + node.inValues.get(i)+"********************************");
		 ...
		 {
			  int sign   			  =   node.operater;//tree.getChild(1).getType();
			  short tempValue =   Bytes.toShort(rp.getColumnData(node.columnName,rowData));
			  int value = 0;
			  if(tempValue < 0)
			  {
		 ...
		 {
			  int sign   			  =   node.operater;//tree.getChild(1).getType();
			  int tempValue =   Bytes.toInt(rp.getColumnData(node.columnName,rowData));

			  long value = 0;
			  if(tempValue < 0)
			  {
				  value = tempValue & 0x0FFFFFFFFl;
			  }
			  else
			  {
			  value = tempValue;
...
//							  System.out.println("RowFilter: in  value is " + value +"**** is  " +node.inValues.get(i));
				  ...
				  int sign   			  =   node.operater;//tree.getChild(1).getType();
				   ...
				   if(node.isFunction)
				   {
					   value = Bytes.toLong(SupportFunction.functionCall(node.funtionName,node.funtionParaNameArrayList,rp.getColumnData(node.columnName,rowData)));
					    ...
					    value = Bytes.toLong(rp.getColumnData(node.columnName,rowData));
				   }
				  switch(sign)
				  {
				  case CreateTestLexer.EQUAL :
		   ...
		   private boolean  atomStringProcess(Node node) throws Exception
		   {
			   ...
			   byte[] data = rp.getColumnDataNoRever(node.columnName,rowData);
			   //System.out.println(Bytes.toString(data));
			   
			   if(node.operater==CreateTestLexer.LIKE)
			   {
				   
				   return stringLike.getValue(Bytes.toString(data),node.strValue);
			   ...
			   if(node.operater==CreateTestLexer.EQUAL)
			   {
				   ...
				   if(node.strValue.equals("'"+Bytes.toString(data).trim()+"'"))
				   {
					   return true;
				   }
				   return false;
			   ...
			   {	
			      ...
			      //处理 in, not in
			   	  String stringValue = Bytes.toString(data).trim();
